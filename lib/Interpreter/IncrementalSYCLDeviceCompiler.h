...
namespace cling {
  ...
  ///\brief Base class for IncrementalSYCLDeviceCompiler. Used only when
  ///lauching
  /// cling without -fsycl.
  ...
  class IncrementalSYCLDeviceCompiler
      : public IncrementalSYCLDeviceCompilerBase {
  public:
  ...
  private:
    ...
    /// setTransaction() more easily. The vector will be cleared when user
    /// inputs new code.
    std::vector<size_t> m_Uniques;
    ///\brief Unique number for the last DumpCodeEntry.
    int lastUnique = -1;
    ///\brief True if the code is entered by the user. Used to avoid non-user
    ///code
  ...
  public:
    ...
    ///\brief Top level compile() called by Interpreter. Will call
    ///refactorCode()
    ...
    ///\param [in] isStatement - 1 if the code is not declaration, and 1
    ///otherwise \param [in] declSuccess - true if declaration is successful
    ///
  ...
  private:
    ///\brief Dump the code of every DumpCodeEntry in EntryList to a target
    ///file.
    ///
    ///\param [in] target - Filename of the target file.
    void dump(const std::string& target);

    ///\brief Call SYCL compiler to generate .spv and kernel info header. Load
    ///the
    /// header into cling.
    ///
    ///\returns True if compiling is successful.
    ...
    ///
    ///\param [in] isStatement - 1 if the code is not declaration, and 1
    ///otherwise \param [in] input - Content of the input code. \param [in] T -
    ///Transaction correspondent to the code.
    void insertCodeEntry(unsigned int is_statement, const std::string& input,
                         Transaction* T);
    ///\brief Use a CompilerInstance to build a AST and use custom ASTConsumer
    ///to
